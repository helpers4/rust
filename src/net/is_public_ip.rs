...
///   including the cloud metadata address), `172.16.0.0/12`, `192.0.0.0/24`, `192.0.2.0/24`,
///   `192.88.99.0/24`, `192.168.0.0/16`, `198.18.0.0/15`, `198.51.100.0/24`, `203.0.113.0/24`,
///   and `224.0.0.0/3` (multicast, reserved and broadcast).
/// - **IPv6** is only accepted inside the global unicast space `2000::/3`, minus `2001::/23`
///   (IETF assignments, including Teredo), `2001:db8::/32` and `3fff::/20` (documentation) and
///   `2002::/16` (6to4). Loopback, unspecified, unique-local, link-local, multicast and the
///   deprecated IPv4-compatible `::a.b.c.d` form are all outside it.
...
///
/// This is only one half of an SSRF defence. Check the address the connection will actually use:
/// resolve the name once, validate that address, and connect to it (not to the name again, which
/// invites DNS rebinding), and validate every redirect target the same way.
///
/// # Examples
///
/// ```
